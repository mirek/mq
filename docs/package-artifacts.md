...
versions, Node engine, and `mq` bin mapping. It pins the core root exports and
verifies that core internals and all CLI JavaScript modules reject deep imports.
Every JavaScript source map embeds its source; declaration-map sources resolve
to packaged `src/` files.
...
the installed binary has an executable mode, runs a real stdin query, and
executes every documented workflow. This prevents passing builds from relying
on workspace links, repository TypeScript sources outside the package, or the
root binary shim.
